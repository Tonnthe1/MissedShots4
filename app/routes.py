...
def search_event():
    if 'user_id' in session:
        if request.method == 'POST':
            ...
            events = Event.query.filter(Event.name.contains(query)).all()
...
def create_event():
    if 'user_id' in session:
        if request.method == 'POST':
            ...
            new_event = Event(name=name, address=address, time=datetime.datetime.strptime(time, '%Y-%m-%dT%H:%M'), online=online, creator_id=creator_id)
